...
fn main() {
    ...
    for path in paths {
        ...
        if path.file_type().unwrap().is_dir() {
            for p in fs::read_dir(path.path()).unwrap() {
                ...
                if file_name == "blog.md" {
                    post_string = fs::read_to_string(p.path()).unwrap();
                }
                if file_name == "images" {
        ...
        while let (Some(s_idx), Some(e_idx)) = (post_string.find("{{ images/"), post_string.find(" }}")) {
            // {{ images/[id]_* }}
            let input_string = &post_string[s_idx..e_idx + 3];
            ...
            let image_ref_string = format!("\"#\n }}</div>\n<img src={{\"build/{post_name}/images/{image_name}\"}}/>\n<div markdown=\"span\">{{ r#\"\n");

            post_string.replace_range(s_idx..e_idx + 3, &image_ref_string);
        }
        post_string.push_str("</div></span>");
